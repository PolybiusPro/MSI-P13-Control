...
def cmd_display_desktop(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "capture", False):
        try:
            with ArtinchipDisplay(rotate=stream["rotate"]) as disp:
                print("Mirroring desktop via screen capture (Ctrl+C to stop)...")
                disp.run_desktop(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_dd = disp_sub.add_parser(
        "desktop",
        help="Mirror desktop to the P13 via EVDI virtual monitor (60fps)",
    )
    ...
    p_dd.add_argument(
        "--capture",
        action="store_true",
        help="fallback: mirror via screen capture instead of virtual monitor",
    )
